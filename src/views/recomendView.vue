  ...
  <div class="detail">
    ...
    <div class="currentPage" v-for="(item, key) in lists" :key="item.id">
      ...
      <div class="footer">
        <span
          class="like"
          @click.native="likeAnddislike(item.id, item.isLiked, item.likeSum)"
        >
...
export default {
  ...
  methods: {
          ...
          if (res.code == "00000") {
            ...
            if (this.lists != []) {
              this.lists.forEach((item) => {
                if (item.isLiked == 1) {
                  this.like[item.id] = true;
                  this.likeId.push(item.id);
                }
              });
            }
    ...
    getAttention() {
      ...
      store.commit("attentionListId", this.attentionListId);
    },
    getname(e, id) {
    ...
    likeAnddislike(id) {
      if (this.likeId.indexOf(id) == -1) {
        this.likeId.push(id);
        this.likelike(id);
      } else {
        this.Cancelike(id);
        this.likeId = this.likeId.filter(function (item) {
          return item != id;
        });
      }
      store.commit("setLikeId", this.likeId);
      store.commit("setLike", this.like);
    },
